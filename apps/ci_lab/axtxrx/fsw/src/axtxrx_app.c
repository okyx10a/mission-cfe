...
int32 AXTXRX_RcvMsg(int32 iBlocking)
{
	...
	{
            case AXTXRX_CMD_MID:
                printf("Recieved a Command Message and sending the message over the software bus.\n");
            ...
            case AXTXRX_SEND_HK_MID:
               printf("Received a HouseKeeping Message and sending the message over the software bus.\n");
               AXTXRX_ReportHousekeeping();
    ...
    else
    {
        ...
        */
        //CFE_EVS_SendEvent(AXTXRX_PIPE_ERR_EID, CFE_EVS_ERROR,"AXTXRX: SB pipe read error (0x%08X), app will not exit", iStatus);
...
void AXTXRX_AppMain()
{
    ...
    {
        AXTXRX_RcvMsg(500);
        /* 0x18C0 is the ID of the application we are going to communicate with */
        CFE_SB_InitMsg ((CFE_SB_Msg_t*)&meow, 0x18C0, sizeof(MeowMan_t), TRUE);
        int32 ret = CFE_SB_SendMsg  ((CFE_SB_Msg_t*)&meow);  
        if(ret != CFE_SUCCESS) {
            printf("Sending Message is a Failure.\n");
        }
        break;
    }
